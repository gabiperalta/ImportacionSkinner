Attribute VB_Name = "CRyAsociados"
Option Explicit
Public Sub ImportarCRyASOCIADOS()
...
cn.Execute "DELETE FROM bandejadeentrada.dbo.ImportaDatos926" 'where idcampana=" & 926 & " and idcia=" & 10002008
...
'=====================================================
 ...
 '      -------------------------------------------------------
        vCampo = "NroPoliza"
        vPosicion = 1
        vgNROPOLIZA = Mid(sLine, vPosicion, 16)
 '      -------------------------------------------------------
        vCampo = "NroSecuencial"
        vPosicion = 17
        vgNROSECUENCIAL = Mid(sLine, vPosicion, 3)
 '      -------------------------------------------------------
 ...
 '      -------------------------------------------------------
        vCampo = "LOCALIDAD"
        vPosicion = 125
        vgLOCALIDAD = Mid(sLine, vPosicion, 30)
 '      -------------------------------------------------------
        vCampo = "PROVINCIA"
        vPosicion = 155
        vgPROVINCIA = Mid(sLine, vPosicion, 15)
 '      -------------------------------------------------------
        vCampo = "CODIGOPOSTAL"
        vPosicion = 170
 ...
 '      -------------------------------------------------------
        ...
        vPosicion = 178
        vgDia = Mid(sLine, vPosicion, 2)
        vgMes = Mid(sLine, vPosicion + 2, 2)
        vgAno = Mid(sLine, vPosicion + 4, 4)
        vgFECHAVIGENCIA = vgDia & "/" & vgMes & "/" & vgAno
        
        
     '   vgFECHAVIGENCIA = Mid(sLine, vPosicion, 2) & "/" & Mid(sLine, vPosicion + 2, 2) & Mid(sLine, vPosicion + 4, 4)
 '      -------------------------------------------------------
        ...
        vgMes = Mid(sLine, vPosicion + 2, 2)
        vgAno = Mid(sLine, vPosicion + 4, 4)
        vgFECHAVENCIMIENTO = vgDia & "/" & vgMes & "/" & vgAno
       ' vgFECHAVENCIMIENTO = Mid(sLine, vPosicion, 2) & "/" & Mid(sLine, vPosicion + 2, 2) & Mid(sLine, vPosicion + 4, 2)
 '      -------------------------------------------------------
 ...
 '      -------------------------------------------------------
        vCampo = "MODELO"
        vPosicion = 224
        vgMODELO = Mid(sLine, vPosicion, 35)
 '      -------------------------------------------------------
        vCampo = "AÑO"
        vPosicion = 259
 ...
 '      -------------------------------------------------------
        vCampo = "PATENTE"
        vPosicion = 263
        vgPATENTE = Mid(sLine, vPosicion, 8)
 '      -------------------------------------------------------
        vCampo = "TipodeServicio"
        vPosicion = 271
        vgTipodeServicio = Mid(sLine, vPosicion, 4)
 '      -------------------------------------------------------
         vCampo = "TIPODEVEHICULO"
        vPosicion = 275
        If Trim(Mid(sLine, vPosicion, 2)) = "A" Then
            vgTIPODEVEHICULO = 1
        ElseIf Trim(Mid(sLine, vPosicion, 2)) = "T" Then
            vgTIPODEVEHICULO = 2
        ElseIf Trim(Mid(sLine, vPosicion, 2)) = "P" Then
            vgTIPODEVEHICULO = 3
        ElseIf Trim(Mid(sLine, vPosicion, 2)) = "M" Then
            vgTIPODEVEHICULO = 5
        Else
            vgTIPODEVEHICULO = 0
        End If
 '      -------------------------------------------------------
        vCampo = "IDPRODUCTO"
        vPosicion = 277
        vgIdProducto = Mid(sLine, vPosicion, 2)
        ObtenerCoberturas (vgidCampana), vgIdProducto, vCantDeErrores, flnErr, Ll, sName
'        If vgIdProducto = "01" Then 'asistencia hogar medica via publica
'            vgCOBERTURAVEHICULO = 9
'            vgCOBERTURAVIAJERO = 2
'            vgCOBERTURAHOGAR = 2
'        ElseIf vgIdProducto = "02" Then ' 'asistencia hogar medica via publica vehiculo
'            vgCOBERTURAVEHICULO = 1
'            vgCOBERTURAVIAJERO = 1
'            vgCOBERTURAHOGAR = 1
'        ElseIf vgIdProducto = "03" Then 'solo vehiculo
'            vgCOBERTURAVEHICULO = 3
'        End If
...
'-=================================================================================================================
            ...
            ssql = "Insert into bandejadeentrada.dbo.ImportaDatos926 ("
            ssql = ssql & "IDPOLIZA, "
            ssql = ssql & "IDCIA, "
