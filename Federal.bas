Attribute VB_Name = "Federal"
Option Explicit
Public Sub ImportarFederal()
...
'        vgIDAUTO = Mid(sLine, 1, 10)
 ...
 '      -------------------------------------------------------
        ...
        vPosicion = 275
        If Mid(sLine, 275, 2) = "01" Then
            vgTIPODEVEHICULO = 1
        ElseIf Mid(sLine, 275, 2) = "02" Then
            vgTIPODEVEHICULO = 2
        Else
            vgTIPODEVEHICULO = 0
        End If
...
'        vgGama = Mid(sLine, 1, 10)
        ...
        DoEvents
    Loop
    ImportadordePolizas.txtProcesando.Text = "Importando " & ImportadordePolizas.cmbCia.Text & Chr(13) & " copiando linea " & Ll - 1 & Chr(13) & " de " & vlineasTotales & " Procesando los datos"
    ...
    For lLote = 1 To vLote
        ...
        Else
                ImportadordePolizas.txtProcesando.Text = "Procesando " & ImportadordePolizas.cmbCia.Text & Chr(13) & " procesando linea " & (lLote * LongDeLote) & Chr(13) & " Procesando los datos"
                DoEvents
        End If
        rsCMP.Close
    Next lLote
