Attribute VB_Name = "ImportadorGenericoExcelSinFechas"
Option Explicit

Public Sub ImportarGenericoExcelSinFechas()
...
'===='Comienzo de lectura del excel======================================
        ...
        Select Case UCase(Trim(sName))
                ...
                Case "FECHANACIMIENTO"
                    If Not IsDate(v) Then
                        vgFechaDeNacimiento = fechaActual
                    Else
    ...
    If vgFECHAVIGENCIA = "00:00:00" And vgFECHAVENCIMIENTO <> "00:00:00" Then 'si no vino vigencia y vino vencimiento
        vgFECHAVIGENCIA = fechaActual
    ElseIf vgFECHAVIGENCIA = "00:00:00" And vgFECHAVENCIMIENTO = "00:00:00" Then 'si no vino vigencia ni vencimiento
       vgFECHAVIGENCIA = fechaActual
        vgFECHAVENCIMIENTO = DateAdd("yyyy", 1, vgFECHAVIGENCIA)
    ElseIf vgFECHAVIGENCIA <> "00:00:00" And vgFECHAVENCIMIENTO = "00:00:00" Then 'si vino vigencia y no vino vencimiento
        vgFECHAVENCIMIENTO = DateAdd("yyyy", 1, vgFECHAVIGENCIA)
    End If
    ...
    '       para indicar que haria que cambiarle en produccion la corrida y la feca de corrida
        ...
        vgIDPOLIZA = 0
                If Not rscn1.EOF Then
                    cambios_de_fechas = 0   'setea la variale de control de fechas cero
                    ...
                    If vgFECHAVIGENCIA Then
                        If Trim(rscn1("FECHAVIGENCIA")) <> Trim(vgFECHAVIGENCIA) Then cambios_de_fechas = cambios_de_fechas + 1
                    End If
                    If IsDate(rscn1("FECHABAJAOMNIA")) Then cambios_de_fechas = cambios_de_fechas + 1
                    If vgFECHAVENCIMIENTO Then
                        If Trim(rscn1("FECHAVENCIMIENTO")) <> Trim(vgFECHAVENCIMIENTO) Then cambios_de_fechas = cambios_de_fechas + 1
                    End If
    ...
    '=========='insert que se hace a la tabla temporal que se crea al comienzo==================
            ...
            ssql = ssql & Trim(cambios_de_fechas) & "', '"
            ssql = ssql & Trim(vdif) & "') "
            cn.Execute ssql
